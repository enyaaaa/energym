...
import { Class, Comment } from "../../utils/types";
...
import { Deletealert } from "./deletecomment";
...
const userbooking = () => {
  ...
  const [reviews, setReviews] = useState<Array<Comment>>([]);
  ...
  useEffect(() => {
    forumapi.get(`api/commentbyuser/${user_id}`).then((res) => {
  ...
  const handleBookingDelete = (e: any, id: any) => {
    e.preventDefault();
  ...
  //function when user press on delete
  const handleReviewDelete = (e: any, id: any) => {
    e.preventDefault();

    forumapi.delete(`/api/comment/${id}`).then((res) => {
      if (res.data.status === 200) {
        toast({
          title: res.data.message,
          status: "success",
  ...
  return (
    <Container>
      ...
      <SimpleGrid minChildWidth="300px" spacing="30px" padding={"5%"}>
              ...
              <Box p="6">
                <Flex justifyContent={"space-between"}>
                  ...
                  <Deletealert
                    ...
                    trigger={(e: any) => handleBookingDelete(e, Classes.id)}
                  />
                </Flex>
                <Box
                  mt="1"
